...
parser.add_argument('--dataset_name', type=str, default='PEAD', help='dataset_name')
parser.add_argument('--root_path', type=str, default='../CBCT_DATA/', help='Name of Dataset')
...
parser.add_argument('--labelnum', type=int, default=8, help='labeled trained samples')
...
def create_model(name='Unet3D', num_classes=3):
    ...
    if name == "resnet34":
        net = Resnet34(n_channels=1, n_classes=num_classes)
    return net
...
def train(train_list, val_list, test_list, fold_id):
    ...
    train_img = [f"{patient_name}.nii.gz" for patient_name in train_list]
    train_mask = [f"{patient_name}_label.nii.gz" for patient_name in train_list]
    ...
    train_dict = [
        {'img': os.path.join(data_path, img), 'mask': os.path.join(data_path, mask)} for
        img, mask in zip(train_img, train_mask)]
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(train_loader):
            volume_batch, label_batch = sampled_batch['img'], sampled_batch['mask']
...
if __name__ == "__main__":
    ...
    load_split_path = r"./data/split_0.pkl"
